package aero.nettracer.web.westjet.testing;
...
public class WS_SeleniumTest extends DefaultSeleneseTestCase {
	
	protected static final String COMPANY_CODE = "WS";
	protected static final String LZ_STATION = "YYC";
	protected static final String BASE_URL = Settings.APP_URL_LOCAL + Settings.PORT1 + "westjet/";
	protected static final String ADMIN_GROUP_ID = "151";
	...
	protected boolean navigateToIncidentAuditTrailTest() {
		return IncidentUtil.navigateToIncidentAuditTrailTest(driver, MENU_ADMIN_AUDIT, Settings.INCIDENT_ID_WS);
	}
	
	protected boolean navigateToIncidentAuditTrail() {
		IncidentUtil.navigateToIncidentAuditTrail(driver, BASE_URL, Settings.INCIDENT_ID_WS);
		return true;
	}
	...
	protected boolean navigateToIncidentTest(String incidentType) {
		return IncidentUtil.navigateToIncidentTest(driver, MENU_LD_SEARCH, incidentType, Settings.INCIDENT_ID_WS);
	}
	
	protected boolean navigateToIncident(String incidentType) {
		IncidentUtil.navigateToIncident(driver, BASE_URL, Settings.INCIDENT_ID_WS);
		return true;
	}
	
	protected boolean navigateToOHDAuditTrailTest() {
		return OnhandUtil.navigateToOHDAuditTrailTest(driver, MENU_ADMIN_AUDIT, Settings.ONHAND_ID_WS);
	}
	
	protected boolean navigateToOHDAuditTrail() {
		OnhandUtil.navigateToOHDAuditTrail(driver, BASE_URL, Settings.ONHAND_ID_WS);
		return true;
	}
	
	protected boolean navigateToOnhandTest() {
		return OnhandUtil.navigateToOHDTest(driver, MENU_OHD_SEARCH, Settings.ONHAND_ID_WS);
	}
	
	protected boolean navigateToOnhand() {
		OnhandUtil.navigateToOHD(driver, BASE_URL, Settings.ONHAND_ID_WS);
		return true;
	}
	...
	protected boolean setPermissions(String[] permissions, boolean[] values) {
		...
		success = PermissionsUtil.setPermissions(ogDriver, BASE_URL, COMPANY_CODE, ADMIN_GROUP_ID, permissions, values) && success;
