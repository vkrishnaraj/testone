...
#Qualified Mileage Deliveries Summary - Is based on and returns GMT Time. Awaiting clarification on Fedex vs NonFedex deliveries
select s.stationCode, count(*) as deliveries, sum(e.checkamt) as cost
  #(case b.incident_ID when null then b.OHD_ID else b.incident_ID end) as reference, b.deliverydate, a.username, concat(p.lastname,", ",p.firstname) as passenger, 
  #d.integration_key , e.checkamt
  from bdo b inner join agent a on b.agent_ID = a.Agent_ID
  inner join bdo_passenger p on p.bdo_ID = b.BDO_ID
  inner join delivercompany d on d.delivercompany_ID = b.delivercompany_ID
...
#BSO Agent Audit Report - Is based on and returns GMT Time
...
#On Hand Scanner Usage Summary - Is based on and returns GMT Time. Awaiting clarification on if we need to capture Auto vs Manual scan
...
#On Hand Scanner Usage Detail - Is based on and returns GMT Time
...
#Southwest LUV Voucher Summary Report - Is based on and returns GMT Time. Incomplete as we do not know where baggage Compensation should come from
