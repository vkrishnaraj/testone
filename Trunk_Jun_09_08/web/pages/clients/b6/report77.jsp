...
<tr>
	<td>
		<bean:message key="colname.incident_date_range" />
		(
	...
	<td>
		...
		<img src="deployment/main/images/calendar/calendar_icon.gif"
			id="calendar" name="calendar" height="15" width="20" border="0"
			onmouseover="this.style.cursor='hand'"
			onClick="cal1xx.select(document.statReportForm.starttime,'calendar','<%=agent.getDateformat().getFormat()%>'); return false;">
		-
		...
		<img src="deployment/main/images/calendar/calendar_icon.gif"
			...
			onmouseover="this.style.cursor='hand'"
			onClick="cal1xx.select(document.statReportForm.endtime,'calendar2','<%=agent.getDateformat().getFormat()%>'); return false;">
	</td>
</tr>
<tr>
  	<td>
    <bean:message key="prepop.search.inc.create_station" />
    :
  	</td>
  	<td>
    <html:select property="createstation_ID" styleClass="dropdown" multiple="true">
      <html:option value="0">
...
<tr>
  	<td>
    <bean:message key="prepop.search.inc.charge_station" />
    :
  	</td>
  	<td>
...
<tr>
	<td>
		<bean:message key="colname.passenger_departure_flight_date_range" />
		(
		<%=agent.getDateformat().getFormat()%>) :
	</td>
	<td>
		<html:text property="paxflightstarttime" size="11" maxlength="10"
			styleClass="textfield" />
		<img src="deployment/main/images/calendar/calendar_icon.gif"
			id="calendar" name="calendar" height="15" width="20" border="0"
			onmouseover="this.style.cursor='hand'"
			onClick="cal1xx.select(document.statReportForm.paxflightstarttime,'calendar','<%=agent.getDateformat().getFormat()%>'); return false;">
		-
		<html:text property="paxflightendtime" size="11" maxlength="10"
			styleClass="textfield" />
		<img src="deployment/main/images/calendar/calendar_icon.gif"
			id="calendar2" name="calendar2" height="15" width="20" border="0"
			onmouseover="this.style.cursor='hand'"
			onClick="cal1xx.select(document.statReportForm.paxflightendtime,'calendar2','<%=agent.getDateformat().getFormat()%>'); return false;">
	</td>
</tr>
<tr>
	<td>
		<bean:message key="prepop.search.inc.assigned_station" />
		:
	</td>
	<td>
		<html:select property="station_ID" styleClass="dropdown"
			...
			<html:options collection="airlineallstationlist"
				property="stationcode" labelProperty="stationcode" />
		</html:select>
	</td>
</tr>
<tr>
