...
public class Dispatcher {
	...
	UploadResponse doPost(final HttpServletRequest request) {
		...
		String funcNum = context.getCkEditorFuncNumStr();
		
		UploadResponse uploadResponse = null;
		// check permissions for user actions
		if (!RequestCycleHandler.isFileUploadEnabled(request))
			uploadResponse = UploadResponse.getFileUploadDisabledError(funcNum);
		// check parameters  
		else if (!Command.isValidForPost(context.getCommandStr()))
			uploadResponse = UploadResponse.getInvalidCommandError(funcNum);
		else if (!ResourceType.isValidType(context.getTypeStr()))
			uploadResponse = UploadResponse.getInvalidResourceTypeError(funcNum);
		else if (!UtilsFile.isValidPath(context.getCurrentFolderStr()))
			uploadResponse = UploadResponse.getInvalidCurrentFolderError(funcNum);
		else {
			...
			try {
				...
				if (type.isDeniedExtension(FilenameUtils.getExtension(fileName)))
					uploadResponse = UploadResponse.getInvalidFileTypeError(funcNum);
				// Secure image check (can't be done if QuickUpload)
					...
					if (sanitizedFileName.equals(newFileName))
						uploadResponse = UploadResponse.getOK(funcNum, fileUrl);
					else {
						uploadResponse = UploadResponse.getFileRenamedWarning(funcNum, fileUrl, newFileName);
				...
				uploadResponse = UploadResponse.getFileUploadWriteError(funcNum);
			} catch (IOException e) {
				uploadResponse = UploadResponse.getFileUploadWriteError(funcNum);
