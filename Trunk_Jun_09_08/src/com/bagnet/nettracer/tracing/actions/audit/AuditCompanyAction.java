...
import com.bagnet.nettracer.tracing.forms.audit.AuditCompanyForm;
 ...
 * @author Ankur Gupta
 */
public class AuditCompanyAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		AuditCompanyForm daform = (AuditCompanyForm) form;
		...
		if ((resultlist = AuditCompanyUtils.getAudits(user.getStation().getCompany()
				.getCompanyCode_ID(), 0, 0, true)) == null
				|| resultlist.size() <= 0) {
			...
			ActionMessage error = new ActionMessage("error.no.company");
			...
			List searchList = AuditCompanyUtils.getAudits(user.getStation().getCompany()
					.getCompanyCode_ID(), rowsperpage, currpage, false);
			...
			for (int i = 0; i < searchList.size(); i++) {
				Audit_Company ic = (Audit_Company) searchList.get(i);
				ic.set_DATEFORMAT(user.getDateformat().getFormat());
				ic.set_TIMEFORMAT(user.getTimeformat().getFormat());
				ic.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils.getTimeZoneById(user.getDefaulttimezone())
						.getTimezone()));
			}
			request.setAttribute("auditcompanylist", searchList);
		}
		return (mapping.findForward(TracingConstants.AUDIT_COMPANY_DETAIL));
	}
}