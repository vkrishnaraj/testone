 ...
 * @author Ankur Gupta
 */
public class AuditAgentAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		AuditAgentForm daform = (AuditAgentForm) form;
		if (request.getParameter("detail") != null) {
			String agent_id = request.getParameter("agent_ID");
			request.setAttribute("agent_ID", agent_id);

			String audit_agent_id = request.getParameter("audit_id");
			if (audit_agent_id != null && audit_agent_id.length() > 0) {
				List compareList = AuditAgentUtils.getAuditsForComparison(audit_agent_id);
				for (int i = 0; i < compareList.size(); i++) {
					Audit_Agent ic = (Audit_Agent) compareList.get(i);
				...
				return (mapping.findForward(TracingConstants.AUDIT_AGENT_COMPARE_DETAIL));
			}
			...
			if ((resultlist = AuditAgentUtils.getAudits(agent_id, 0, 0, true)) == null
					|| resultlist.size() <= 0) {
				...
				List searchList = AuditAgentUtils.getAudits(agent_id, rowsperpage, currpage, false);
				...
				for (int i = 0; i < searchList.size(); i++) {
					Audit_Agent ic = (Audit_Agent) searchList.get(i);
					ic.set_DATEFORMAT(user.getDateformat().getFormat());
					ic.set_TIMEFORMAT(user.getTimeformat().getFormat());
					ic.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils
							.getTimeZoneById(user.getDefaulttimezone()).getTimezone()));
				}
				request.setAttribute("auditagentlist", searchList);
				return (mapping.findForward(TracingConstants.AUDIT_AGENT_DETAIL));
			}
		}

		if (request.getParameter("search") == null) {
			return (mapping.findForward(TracingConstants.AUDIT_AGENT));
		}

		List resultlist = null;
		// get number of records found
		if ((resultlist = AuditAgentUtils.findAgentsByAuditSearchCriteria(daform, user, 0, 0, true)) == null
				|| resultlist.size() <= 0) {
			int rowsperpage = request.getParameter("rowsperpage") != null ? Integer.parseInt(request
					.getParameter("rowsperpage")) : TracingConstants.ROWS_PER_PAGE;
			if (rowsperpage < 1) rowsperpage = TracingConstants.ROWS_PER_PAGE;
			...
			//find the paginated on hand bags
			List searchList = AuditAgentUtils.findAgentsByAuditSearchCriteria(daform, user, rowsperpage,
					currpage, false);
			...
			request.setAttribute("agentList", searchList);
		}
		return (mapping.findForward(TracingConstants.AUDIT_AGENT));
	}
}