 ...
 * @author Ankur Gupta
 */
public class AuditOnHandAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		AuditOnHandForm daform = (AuditOnHandForm) form;
		if (request.getParameter("detail") != null) {
			String ohd_id = request.getParameter("ohd_ID");
			request.setAttribute("ohd_ID", ohd_id);

			String audit_ohd_id = request.getParameter("audit_ohd_id");
			if (audit_ohd_id != null && audit_ohd_id.length() > 0) {
				List compareList = AuditOHDUtils.getAuditsForComparison(audit_ohd_id);
				for (int i = 0; i < compareList.size(); i++) {
					Audit_OHD ic = (Audit_OHD) compareList.get(i);
				...
				return (mapping.findForward(TracingConstants.AUDIT_ONHAND_COMPARE_DETAIL));
			}
			...
			if ((resultlist = AuditOHDUtils.getAudits(ohd_id, 0, 0, true)) == null
					|| resultlist.size() <= 0) {
				...
				List searchList = AuditOHDUtils.getAudits(ohd_id, rowsperpage, currpage, false);
				...
				for (int i = 0; i < searchList.size(); i++) {
					Audit_OHD ic = (Audit_OHD) searchList.get(i);
					ic.set_DATEFORMAT(user.getDateformat().getFormat());
					ic.set_TIMEFORMAT(user.getTimeformat().getFormat());
					ic.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils
							.getTimeZoneById(user.getDefaulttimezone()).getTimezone()));
				}
				request.setAttribute("auditonhandlist", searchList);
				return (mapping.findForward(TracingConstants.AUDIT_ONHAND_DETAIL));
			}
		}

		if (request.getParameter("search") == null) {
			return (mapping.findForward(TracingConstants.AUDIT_ONHAND));
		}

		List resultlist = null;
		// get number of records found
		if ((resultlist = AuditOHDUtils.findOnHandBagsByAuditSearchCriteria(daform, user, 0, 0, true)) == null
				|| resultlist.size() <= 0) {
			int rowsperpage = request.getParameter("rowsperpage") != null ? Integer.parseInt(request
					.getParameter("rowsperpage")) : TracingConstants.ROWS_PER_PAGE;
			if (rowsperpage < 1) rowsperpage = TracingConstants.ROWS_PER_PAGE;
			...
			//find the paginated on hand bags
			List searchList = AuditOHDUtils.findOnHandBagsByAuditSearchCriteria(daform, user,
