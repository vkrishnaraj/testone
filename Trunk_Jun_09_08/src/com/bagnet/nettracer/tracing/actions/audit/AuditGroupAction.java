 ...
 * @author Ankur Gupta
 */
public class AuditGroupAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		AuditGroupForm daform = (AuditGroupForm) form;
		if (request.getParameter("detail") != null) {
			String userGroup_ID = request.getParameter("userGroup_ID");
			request.setAttribute("userGroup_ID", userGroup_ID);

			String audit_id = request.getParameter("audit_id");
			if (audit_id != null && audit_id.length() > 0) {
				List compareList = AuditGroupUtils.getAuditsForComparison(audit_id);
				for (int i = 0; i < compareList.size(); i++) {
					Audit_UserGroup ic = (Audit_UserGroup) compareList.get(i);
				...
				return (mapping.findForward(TracingConstants.AUDIT_GROUP_COMPARE_DETAIL));
			}
			...
			if ((resultlist = AuditGroupUtils.getAudits(userGroup_ID, 0, 0, true)) == null
				...
				ActionMessage error = new ActionMessage("error.no.group");
				...
				List searchList = AuditGroupUtils.getAudits(userGroup_ID, rowsperpage, currpage, false);
				...
				for (int i = 0; i < searchList.size(); i++) {
					Audit_UserGroup ic = (Audit_UserGroup) searchList.get(i);
					ic.set_DATEFORMAT(user.getDateformat().getFormat());
					ic.set_TIMEFORMAT(user.getTimeformat().getFormat());
					ic.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils
							.getTimeZoneById(user.getDefaulttimezone()).getTimezone()));
				}
				request.setAttribute("auditgrouplist", searchList);
				return (mapping.findForward(TracingConstants.AUDIT_GROUP_DETAIL));
			}
		}

		if (request.getParameter("search") == null) {
			return (mapping.findForward(TracingConstants.AUDIT_GROUP));
		}
		...
		if ((resultlist = AuditGroupUtils.findGroupsByAuditSearchCriteria(daform, user, 0, 0, true)) == null
			...
			//find the paginated on hand bags
			List searchList = AuditGroupUtils.findGroupsByAuditSearchCriteria(daform, user, rowsperpage,
					currpage, false);
