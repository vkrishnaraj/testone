 ...
 * @author Ankur Gupta
 */
public class ViewTemporaryOnHand extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		//		 menu highlite
		request.setAttribute("highlite", TracingConstants.SYSTEM_COMPONENT_NAME_TEMPORARY_ON_HAND);

		int ohdListCount = ((Long) OHDUtils.getOHDs(user, sort, (ViewTemporaryOnHandsForm) form,
				"" + TracingConstants.OHD_STATUS_TEMP, "" + agent_station.getStation_ID(), 0, 0, true).get(
				0)).intValue();
		if (ohdListCount > 0) {
			/** ************ pagination ************* */
			int rowcount = -1;
			...
			//request list
			List ohdList = OHDUtils.getOHDs(user, sort, (ViewTemporaryOnHandsForm) form, ""
					+ TracingConstants.OHD_STATUS_TEMP, "" + agent_station.getStation_ID(), rowsperpage,
					currpage, false);
		...
		return mapping.findForward(TracingConstants.VIEW_TEMP_OHD);
	}
}