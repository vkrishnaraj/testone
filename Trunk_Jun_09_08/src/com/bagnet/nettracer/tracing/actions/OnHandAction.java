...
public class OnHandAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (!UserPermissions.hasLinkPermission(mapping.getPath().substring(1)
				...
				&& !UserPermissions.hasPermission(
						TracingConstants.SYSTEM_COMPONENT_NAME_REMARK_UPDATE_OH, user))
		...
		// add new remark box -- set new remark with current gmt time
			...
			r.setCreatetime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
		...
		// save temporary.
		else if ((request.getParameter("savetemp") != null && !request
				.getParameter("savetemp").equals(""))
				|| request.getParameter("savetracing") != null
			...
			if (request.getParameter("savetemp") != null
					&& !request.getParameter("savetemp").equals(""))
			...
			while (e.hasMoreElements()) {
				...
				if (parameter.indexOf("[") != -1) {
					index = parameter.substring(parameter.indexOf("[") + 1, parameter
							.indexOf("]"));
			...
			if (foundAddress) {
				...
				if (UserPermissions.hasPermission(
						TracingConstants.SYSTEM_COMPONENT_NAME_ADD_ON_HAND_BAG, user)) {
					if (user.getStation().getCompany().getCompanyCode_ID().equals(
							addform.getHolding_company())
							&& user.getStation().getStationcode().equals(
									addform.getHolding_station())) {
	...
	private String getReportFile(OnHandForm theform, Agent user,
		...
		String reportfile = createReport(type, sc, theform.getOhd_id(), user,
				selections, request);

		return reportfile;
	}

	private static String createReport(int type, ServletContext sc,
			String ohd_id, Agent user, HashMap selections, HttpServletRequest request) {
		...
		try {
			...
			parameters.put("airline",
					format("" + form.getAgent().getCompanycode_ID()));
			...
			if (form.getXDesc1() > 0) {
				parameters.put("ede1", format(""
						+ TracerUtils.getXdescelement(form.getXDesc1())
								.getDescription()));
			} else {
				parameters.put("ede1", "");
			}
			if (form.getXDesc2() > 0) {
				parameters.put("ede2", format(""
						+ TracerUtils.getXdescelement(form.getXDesc2())
								.getDescription()));
			} else {
				parameters.put("ede2", "");
			}
			if (form.getXDesc3() > 0) {
				parameters.put("ede3", format(""
						+ TracerUtils.getXdescelement(form.getXDesc3())
								.getDescription()));
			} else {
				parameters.put("ede3", "");
			}
			parameters.put("bag_arrive_date",
					format("" + form.getDispBagArriveDate()));
			parameters.put("found_at_company", format("" + form.getFound_company()));
			parameters.put("airline_membership",
					format("" + form.getCompanycode_ID()));
			parameters.put("membership_num", format("" + form.getMembershipnum()));
			parameters.put("membershipstatus",
					format("" + form.getMembershipstatus()));
			parameters
					.put("airline_membership", format("" + form.getMembershipnum()));
			parameters.put("color", format("" + form.getBagColor()));
			if (form.getManufacturer_ID() > 0) {
				parameters.put("manufacturer", format(""
						+ TracerUtils.getManufacturer(form.getManufacturer_ID())
								.getDescription()));
			} else {
			...
			parameters.put("storage_location",
					format("" + form.getStorage_location()));
			...
			if (selections.get("passenger") != null) {
				...
				if (passengers != null && passengers.size() > 0) {
					parameters.put("passengerReport", ReportBMO.getCompiledReport(
							"ohd_passenger", sc.getRealPath("/")));
					parameters.put("addressReport", ReportBMO.getCompiledReport(
							"ohd_address", sc.getRealPath("/")));
					parameters.put("passenger",
							new JRBeanCollectionDataSource(passengers));
				} else
			...
			if (selections.get("itinerary") != null) {
				...
				if (itineraries != null && itineraries.size() > 0) {
					parameters.put("itineraryReport", ReportBMO.getCompiledReport(
							"ohd_itinerary", sc.getRealPath("/")));
					parameters.put("itinerary", new JRBeanCollectionDataSource(
							itineraries));
				} else
			...
			if (selections.get("inventory") != null) {
				...
				if (inventories != null && inventories.size() > 0) {
					parameters.put("inventoryReport", ReportBMO.getCompiledReport(
							"ohd_inventory", sc.getRealPath("/")));
					parameters.put("inventory", new JRBeanCollectionDataSource(
							inventories));
				} else {
			...
			if (selections.get("remarks") != null) {
				...
				if (remarks != null && remarks.size() > 0) {
					parameters.put("remarkReport", ReportBMO.getCompiledReport("remarks",
							sc.getRealPath("/")));
			...
			if (selections.get("messages") != null) {
				...
				if (messages != null && messages.size() > 0) {
					for (Iterator i = messages.iterator(); i.hasNext();) {
						...
						msg.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils.getTimeZoneById(
								user.getDefaulttimezone()).getTimezone()));
					}
					parameters.put("messageReport", ReportBMO.getCompiledReport(
							"messages", sc.getRealPath("/")));
			...
			if (selections.get("tasks") != null) {
				...
				if (tasks != null && tasks.size() > 0) {
					...
					parameters.put("taskReport", ReportBMO.getCompiledReport("tasks", sc
							.getRealPath("/")));
			...
			if (selections.get("forward") != null) {
				...
				if (forwardLog != null && forwardLog.size() > 0) {
					for (Iterator i = forwardLog.iterator(); i.hasNext();) {
						...
						log.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils.getTimeZoneById(
								user.getDefaulttimezone()).getTimezone()));
					...
					parameters.put("forwardItineraryReport", ReportBMO.getCompiledReport(
							"forward_itinerary", sc.getRealPath("/")));
					parameters.put("forwardReport", ReportBMO.getCompiledReport(
							"ohd_forward", sc.getRealPath("/")));
			...
			if (selections.get("request") != null) {
				...
				if (requestList != null && requestList.size() > 0) {
					for (Iterator iter = requestList.iterator(); iter.hasNext();) {
						...
						req.set_TIMEZONE(TimeZone.getTimeZone(AdminUtils.getTimeZoneById(
								user.getDefaulttimezone()).getTimezone()));
					}
					// request reports
					parameters.put("requestReport", ReportBMO.getCompiledReport(
							"ohd_request", sc.getRealPath("/")));
					parameters
							.put("request", new JRBeanCollectionDataSource(requestList));
				} else {
			...
			if (selections.get("matches") != null) {
				...
				if (matches != null && matches.size() > 0) {
					parameters.put("match_detail_report", ReportBMO.getCompiledReport(
							"match_detail_report", sc.getRealPath("/")));
					parameters.put("matchReport", ReportBMO.getCompiledReport(
							"match_report", sc.getRealPath("/")));
			...
			return bmo.getReportFile(t, parameters, "onhand_history", sc
					.getRealPath("/"), type);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
