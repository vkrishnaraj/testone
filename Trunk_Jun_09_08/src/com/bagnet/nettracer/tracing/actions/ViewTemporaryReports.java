 ...
 * @author Ankur Gupta
 */
public class ViewTemporaryReports extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		//		 menu highlite
		request.setAttribute("highlite", TracingConstants.SYSTEM_COMPONENT_NAME_TEMPORARY_REPORTS);

		int incidentListCount = ((Long) IncidentUtils.getIncidents(user, sort,
				(ViewTemporaryReportsForm) form, "" + TracingConstants.MBR_STATUS_TEMP,
				"" + agent_station.getStation_ID(), 0, 0, true).get(0)).intValue();
		if (incidentListCount > 0) {
			/** ************ pagination ************* */
			int rowcount = -1;
			...
			//request list
			List incidentList = IncidentUtils.getIncidents(user, sort, (ViewTemporaryReportsForm) form,
					"" + TracingConstants.MBR_STATUS_TEMP, "" + agent_station.getStation_ID(), rowsperpage,
					currpage, false);
			...
			for (int i = 0; i < incidentList.size(); i++) {
				Incident ic = (Incident) incidentList.get(i);
		...
		return mapping.findForward(TracingConstants.VIEW_TEMP_REPORTS);
	}
}