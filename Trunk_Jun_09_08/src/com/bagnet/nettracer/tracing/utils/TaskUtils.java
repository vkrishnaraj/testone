...
public class TaskUtils {
	...
	public static List getTaskStatusList(String locale) {
		...
		try {
			...
			Criteria cri = sess.createCriteria(Status.class).add(Expression.eq("locale", locale)).add(
					Expression.eq("table_ID", new Integer(4))).addOrder(Order.asc("status_ID"));
			return cri.list();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 * @return the status list
	 */
	public static Status getStatusByDesc(String locale, String desc) {
		Session sess = null;
		try {
			sess = HibernateWrapper.getSession().openSession();
			Criteria cri = sess.createCriteria(Status.class).add(Expression.eq("locale", locale)).add(
					Expression.eq("table_ID", new Integer(4))).add(Expression.eq("description", desc));
	 ...
	 * @return the status list
	 */
	public static Priority getPriority(String locale, String desc) {
		Session sess = null;
		try {
			sess = HibernateWrapper.getSession().openSession();
			Criteria cri = sess.createCriteria(Priority.class).add(Expression.eq("locale", locale)).add(
					Expression.eq("description", desc));
			return (Priority) cri.list().get(0);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
