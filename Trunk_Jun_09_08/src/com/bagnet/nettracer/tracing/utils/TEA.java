 ...
 * <P>
 * <LI>The following description is taken from the Mach5 Software cryptography
 ...
 * 
 * href="http://www.cs.berkeley.edu/~daw/">David Wagner </a>, John Kelsey, and
 * <a
 * 
 ...
 * 16).toByteArray(); TEA t = new TEA(key); <BR>
...
public class TEA {
	...
	public TEA() {
		...
		if (klen != 16) throw new ArrayIndexOutOfBoundsException(this.getClass().getName()
				+ ": Key is not 16 bytes");

		int j, i;
		for (i = 0, j = 0; j < klen; j += 4, i++)
			_key[i] = (key[j] << 24) | (((key[j + 1]) & 0xff) << 16) | (((key[j + 2]) & 0xff) << 8)
					| ((key[j + 3]) & 0xff);
	 ...
	 * parameters with the results. The integers are usually created from 8 bytes.
	 ...
	 * parameters with the results. The integers are usually decocted to 8 bytes.
	 * The decoction of the <code>int</code> s to bytes can be done this way.
	 * 
	 * <PRE>
	 * 
	...
	/**
	 * Byte wrapper for encoding. Converts bytes to ints. Padding will be added if
	 * required.
	 * 
	 * @param b
	...
	public int[] encode(byte b[], int count) {
		...
		for (i = 0, j = 0; j < bLen; j += 8, i += 2) {
			...
			r[0] = (bp[j] << 24) | (((bp[j + 1]) & 0xff) << 16) | (((bp[j + 2]) & 0xff) << 8)
					| ((bp[j + 3]) & 0xff);
			r[1] = (bp[j + 4] << 24) | (((bp[j + 5]) & 0xff) << 16) | (((bp[j + 6]) & 0xff) << 8)
					| ((bp[j + 7]) & 0xff);
			encipher(r);
			out[i] = r[0];
			out[i + 1] = r[1];
	...
	/**
	 * Convert a byte array to ints and then decode. There may be some padding at
	 * the end of the byte array from the previous encode operation.
	 * 
	 * @param b
	 *          bytes to decode
	 * @param count
	...
	public byte[] decode(byte b[], int count) {
		...
		for (i = 0, j = 0; i < intCount; i += 2, j += 8) {
			ini[i] = (b[j] << 24) | (((b[j + 1]) & 0xff) << 16) | (((b[j + 2]) & 0xff) << 8)
					| ((b[j + 3]) & 0xff);
			ini[i + 1] = (b[j + 4] << 24) | (((b[j + 5]) & 0xff) << 16) | (((b[j + 6]) & 0xff) << 8)
					| ((b[j + 7]) & 0xff);
		}
		return decode(ini);
	}
