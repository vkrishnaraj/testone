...
public class RetrieveWTActionFiles extends Thread {
	...
	public void run() {
		while (true) {
			try {
				...
				this.client = WorldTracerUtils.connectWT(WorldTracerUtils.getWt_suffix_airline(company) + "/",company);
				
				List<Station> stationList = AdminUtils.getStations(null, company, 0, 0);
				String station = null;
				for (int x = 0; x < stationList.size(); ++x) {
					...
					for (int i = 1; i < 8; i++) {
					//for (int i = 1; i < 2; i++) {
						parsewt_actionfiles("FW", Integer.toString(i), company, station);
						parsewt_actionfiles("AA", Integer.toString(i), company, station);
						parsewt_actionfiles("WM", Integer.toString(i), company, station);
						parsewt_actionfiles("SP", Integer.toString(i), company, station);
						parsewt_actionfiles("AP", Integer.toString(i), company, station);
						parsewt_actionfiles("EM", Integer.toString(i), company, station);
						parsewt_actionfiles("CM", Integer.toString(i), company, station);
						parsewt_actionfiles("LM", Integer.toString(i), company, station);
						parsewt_actionfiles("PR", Integer.toString(i), company, station);
					}
				}
	...
	public void parsewt_actionfiles(String wt_type, String day, String airline, String station) {
		...
		try {
			...
			stmt.executeUpdate("update worldtracer_actionfiles set delete_trigger = 1");
			...
			while ((parsed = StringUtils.ParseWTString2(result, ac_start, ac_end)) != null) {
				...
				// check to see if this file already exist, if not, then insert, if already exist, then set trigger to 0 so it doesn't get deleted
				rs = stmt.executeQuery("select id from worldtracer_actionfiles where action_file_text = '" + parsed.replace("'", "''") + "'");
				if (rs.next()) {
					// has this action file already, do not update and set delete trigger to 0
					stmt.executeUpdate("update worldtracer_actionfiles set delete_trigger = 0 where id = '" + rs.getInt("id") + "'");
				} else {
					sql = "insert into worldtracer_actionfiles (action_file_type,action_file_text,day,station,airline,wt_incident_id,wt_ohd_id,delete_trigger) values " 
						+ "('" + wt_type.toUpperCase() + "','" + parsed.replace("'", "''") + "','" + day + "','" 
						+ station + "','" + airline + "','" + wt_ahl_id + "','" + wt_ohd_id + "',0)";
					stmt.executeUpdate(sql);
			...
			// now delete all the rows with delete_trigger = 1 and day and type match
			stmt.executeUpdate("delete from worldtracer_actionfiles where delete_trigger = 1 and action_file_type = '" + wt_type.toUpperCase() + "' and day = '" + day + "'");
			sess.disconnect();
