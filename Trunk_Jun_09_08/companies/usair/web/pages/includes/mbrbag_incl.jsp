			...
			<tr>
				...
				<td colspan="2"><bean:message key="colname.claimnum.req2" /> <br>
					<html:text name="theitem" property="claimchecknum" size="10"
						maxlength="10" styleClass="textfield" indexed="true" /></td>
				<%
					} else {
				%>
			...
			<tr>
				<td valign="top"><bean:message key="colname.color.req" /> <a
					...
					onclick="openChart2('pages/popups/bagtypechart.jsp?charttype=2&key=theitem[<%=i%>].bagtype&type=bagtype',800,370,230);return false;"><bean:message
							key="chart2" /></a> <br> <%String bagpos = "bagtype" + i; String funcCall = "checkBagType(" + i + ")"; %>
					...
					<div id="childRestraint<%=i%>" style="display: none">
						<%
							if (UserPermissions
										.hasPermission(
												TracingConstants.SYSTEM_COMPONENT_CHILD_RESTRAINT_SYSTEM,
												a)
										&& theitem.getItemtype_ID() != 2) {
						%>
 ...
 %> <br> <input type="text"
			...
			<tr>
				...
				<td><bean:message key="colname.lvldamage" /> <br> <html:select
						...
						<html:option
							value="<%=String
									.valueOf(TracingConstants.DAMAGE_MINOR)%>">
							<bean:message key="select.minor" />
						</html:option>
						<html:option
							value="<%=String
									.valueOf(TracingConstants.DAMAGE_MAJOR)%>">
							<bean:message key="select.major" />
						</html:option>
						<html:option
							value="<%=String
									.valueOf(TracingConstants.DAMAGE_COMPLETE)%>">
							<bean:message key="select.complete" />
						</html:option>
					</html:select> <%
 	if (request.getAttribute("damaged") != null
 					&& UserPermissions
 							.hasPermission(
 									TracingConstants.SYSTEM_COMPONENT_NAME_ISSUE_REPLACEMENT_BAGS,
 									a)) {
 %>
					<br /> *&nbsp;<bean:message key="replacement.bag.issued" /><br />
					<html:select name="theitem" property="replacementBagIssued"
						styleId="replacementBagIssued" styleClass="dropdown"
						indexed="true">
						<html:option value="-1">
							<bean:message key="select.please_select" />
						</html:option>
						<html:option value="<%=String.valueOf(TracingConstants.YES)%>">
							<bean:message key="select.yes" />
						</html:option>
						<html:option value="<%=String.valueOf(TracingConstants.NO)%>">
 ...
 %></td>
			...
			<%
					...
					if (UserPermissions.hasPermission(
							TracingConstants.SYSTEM_COMPONENT_NAME_DAMAGED_PHOTOS,
							a)) {
			%>
			<tr>
