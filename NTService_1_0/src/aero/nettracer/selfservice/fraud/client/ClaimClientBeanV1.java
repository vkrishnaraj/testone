...
public class ClaimClientBeanV1 implements ClaimClientRemoteV1{
	...
	public TraceResponse traceFile(long fileId, int maxDelay,
		...
		Mapper mapper = DozerBeanMapperSingletonWrapper.getInstance();
		return mapper.map(bean.traceFile(fileId, maxDelay, isPrimary, returnResults), aero.nettracer.fs.model.transport.v1.detection.TraceResponse.class);
	}

	@Override
	public TraceResponse getFileMatches(long fileId) {
		Mapper mapper = DozerBeanMapperSingletonWrapper.getInstance();
		return mapper.map(bean.getFileMatches(fileId), aero.nettracer.fs.model.transport.v1.detection.TraceResponse.class);
	}
	...
	public File getFile(long fileId, String airline) {
		Mapper mapper = DozerBeanMapperSingletonWrapper.getInstance();
		return mapper.map(bean.getFile(fileId, airline), aero.nettracer.fs.model.transport.v1.File.class);
	}
